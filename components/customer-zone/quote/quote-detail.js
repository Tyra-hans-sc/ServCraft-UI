...
function QuoteDetail(props) {
  ...
  const exportDocument = () => {
    DownloadService.downloadFile("GET", `/CustomerZone/GetQuoteDocument?quoteID=${quote.ID}`, null, true, false, props.tenantID, props.customerID, props.apiUrlOverride);
  };
  ...
  return (<>
      ...
      <div className="card">
        ...
        <div className="row">
          <div className="column">
            <h2 className="item-heading">Quote Detail</h2>
          </div>
          <div className="column-end">
            <CellStatus extraClasses="no-min-width" value={quote.QuoteStatus} valueEnum="QuoteStatus" />
          </div>
        </div>
            ...
            <div className={props.mobileView ? "" : "row"}>
              <div className={props.mobileView ? "row" : "column right-padding"}>
                <TextInput
                  ...
                  label="Quote date"
                  required={false}
                  value={quote.QuoteDate ? Time.toISOString(Time.parseDate(quote.QuoteDate), false, false) : ""}
                  extraClasses="white"
                />
              </div>
              <div className={props.mobileView ? "row" : "column left-padding"}>
                <TextInput
                  ...
                  label="Expiry date"
                  required={false}
                  value={quote.ExpiryDate ? Time.toISOString(Time.parseDate(quote.ExpiryDate), false, false) : ""}
                  extraClasses="white"
                />
              </div>
            </div>
    ...
    {showTotals === true ? <div className="row">
      ...
      <div className={props.mobileView ? "column" : "column-end"}>
        <QuoteTotals quote={quote} mobileView={props.mobileView} />
      </div>
    </div> : <></>}
    <style jsx>{`
