...
*/
        ...
        >
            <div style={{ paddingTop: "6px" }}>
                ...
                <span style={{ pointerEvents: "none" }}>
                    {dropdownOpenMantine ?
                        <img src="/specno-icons/chevron_up.svg" alt="" style={{ pointerEvents: "none" }} />
                        :
                        <img src="/specno-icons/chevron_down.svg" alt="" style={{ pointerEvents: "none" }} />
                    }
                </span>
            </div>
    ...
    const buildSelectOptions = () => {
        const base = mapLocalOptionsMantine()
            .filter(({ value, label }) =>
                !mapSelectedOptionsMantine().includes(value) &&
                (label.toLowerCase().trim().includes(search.toLowerCase()) ||
                 value.toLowerCase().trim().includes(search.toLowerCase()))
            );

        if (!groupField) {
            return base.map(({ value, label, dataItem }) => (
                <Combobox.Option value={value} key={value}>
                    <Group wrap={'nowrap'}>
                        <div>{itemRenderMantine({ value, label, dataItem })}</div>
                    </Group>
                </Combobox.Option>
            ));
        }
