...
function AuditLog({recordID, retriggerSearch}) {
  ...
  const retriggerSearchOriginal = useRef(retriggerSearch);
  ...
  useEffect(() => {
    ...
    if (retriggerSearchOriginal.current === retriggerSearch) return;
    retriggerSearchOriginal.current = retriggerSearch;
  ...
  function toggleAuditLogAccordion() {
    setShowAuditLogContents(!showAuditLogContents);
    if (auditLogOpened) {
      setAuditLogOpened(!auditLogOpened);
    } else {
      setTimeout(() => {
        setAuditLogOpened(!auditLogOpened);
      }, 600);
  ...
  return (
    <div className={`${auditLogResults.length > 0 ? 'accordion-container' : 'hidden'}`}>
      ...
      <div className={`accordion-body ${auditLogOpened ? 'accordion-overflow' : 'accordion-body-hidden'}`} ref={auditLogAccordionBodyRef} style={{ maxHeight: `${auditLogHeight}` }}>
        <div className="row">
