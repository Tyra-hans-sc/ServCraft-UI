...
const LS_COL_MAPPING_NAME = 'quote-doc'
...
const QuotePreferencesDrawer: FC<{
        open: boolean;
        onClose: () => void;
            ...
            x => (
                x.ColumnName === 'UnitPriceInclusive' ? {
                    ...
                    x.ColumnName === 'UnitPriceExclusive' ? {
                        ...
                        /*x.ColumnName === 'LineTotalInclusive' ? {
                                ...x,
                                Show: useInclusivePrice,
                            } :
                            x.ColumnName === 'LineTotalExclusive' ? {
                                    ...x,
                                    Show: !useInclusivePrice,
                                } :*/
                                x
    ...
    const handleNewReceivedColumnMapping = (newColumnMapping: ColumnMappingData[] | null) => {
        const columnMapping = /*newColumnMapping || */props.mapping.map((x, i) => {
   ...
   useQuery(['authConfig', Enums.ConfigurationSection.Quote], () => UserConfigService.getSettings(Enums.ConfigurationSection.Quote, Enums.ConfigurationType.CRUD),
        {
    ...
    const authConfigMutation = useMutation(['quoteDocAutConfig'], UserConfigService.saveConfig)
    ...
    const handleColumnMappingChange = (cm: ColumnMappingData[], itemChanged?: {key: string, checked: boolean}) => {
        ...
        /*if(itemChanged?.key === 'UnitPriceExclusive' || itemChanged?.key === 'UnitPriceInclusive' || itemChanged?.key === 'UnitPriceMarkup') {
            const exclusiveShown = newColMapping.find(x => x.ColumnName === 'UnitPriceExclusive')?.Show
            const inclusiveShown = newColMapping.find(x => x.ColumnName === 'UnitPriceInclusive')?.Show
            newColMapping = newColMapping.map(x =>
                (x.ColumnName === 'UnitPriceExclusive') ? {
                    ...x,
                    Disabled: inclusiveShown,
                } : x.ColumnName === 'UnitPriceInclusive' ? {
                    ...x,
                    Disabled: exclusiveShown,
                } : x
            );
        }*/
    ...
    return <>
        ...
        >
            ...
            {
                /*<Title order={5} c={'scBlue.9'} mt={'sm'}>
                    <Flex align={'center'} gap={5}>
                        PDF Settings
                    </Flex>
                </Title>*/
            ...
            <Alert
                ...
                // title={`Quote Columns`}
            >
                Updates won&apos;t affect the Quote PDF.
            </Alert>
        </Drawer>
    </>
}

export default QuotePreferencesDrawer
